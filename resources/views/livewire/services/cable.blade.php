 ...
 <section class="w-full">
     <x-page-header :title="__('Purchase Cable Subscription')" :description="__('Purchase cable TV subscriptions below')" />
     ...
     <form method="POST" wire:submit="purchase" class="flex flex-col gap-6">
         ...
         <flux:select
             ...
             :label="__('Cable Plan')"
             :placeholder="__('Select cable plan')"
             required
         ...
         <flux:input
             wire:model="smartcard_number"
             :label="__('Smartcard Number')"
             type="text"
             required
             autofocus
             autocomplete="off"
     ...
     <flux:modal wire:model.self="showConfirmationModal">
         <div class="space-y-6">
             <div>
                 ...
                 <flux:subheading>
                     {{ __('Please review your cable subscription details before confirming.') }}
                 </flux:subheading>
             </div>
             ...
             <div class="flex w-full items-start gap-6">
                 ...
                 <div class="flex-1">
                     <div class="grid grid-cols-1 gap-2">
                         ...
                         @endisset
                         <div class="flex justify-between">
                             <span class="text-gray-600 dark:text-gray-400">Smartcard:</span>
                             <span class="font-semibold">{{ $smartcard_number }}</span>
                         </div>
                     </div>
                 </div>
             </div>

             <p>{{ __('Are you sure you want to purchase this cable subscription?') }}</p>
             <flux:separator />
